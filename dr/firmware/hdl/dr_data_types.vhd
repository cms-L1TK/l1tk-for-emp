    ...
    for k in res.stubs'range loop
      ...
      if s.valid = '1' then
        ...
        if dPhi > 0 then
          ...
          chi2 := chi2 + resize((phi/dPhi)*(phi/dPhi)/2, chi2'length); -- What is done in emulation, I don't think it's right...
        end if;

        if dZ > 0 then
          report "SOMETHING NON ZEROOO Z";
          chi2 := chi2 + resize((z/dZ)*(z/dZ)/2, chi2'length); -- What is done in emulation, I don't think it's right...
        end if;
